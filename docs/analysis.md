...
## Проблемы сырого датасета и предпосылки для очистки

Несмотря на статус стандарта де-факто, сырой датасет **CSE-CIC-IDS2018** содержит ряд критических технических проблем. В нашем проекте реализован строгий пайплайн предварительной очистки (обработка NaN, бесконечностей и удаление дубликатов). Необходимость таких шагов подтверждается исследованиями других авторов:

1. **Критический дисбаланс и отсутствие стандартов чистки:**
В научной работе *«A survey and analysis of intrusion detection models based on CSE-CIC-IDS2018 Big Data» (Leevy et al.)* авторы проанализировали существующие модели и пришли к выводу, что большинство исследователей игнорируют проблему дисбаланса классов, а информация о методах очистки датасета практически отсутствует, что рушит воспроизводимость экспериментов. 
**Исследование:** [ResearchGate / Springer](https://www.researchgate.net/publication/346536711_A_survey_and_analysis_of_intrusion_detection_models_based_on_CSE-CIC-IDS2018_Big_Data)

2. **Ошибки разметки (Labeling Errors):**
Другие исследователи, детально разбиравшие структуру логов, выявили, что в CSE-CIC-IDS2018 присутствует значительная доля ошибок в самой маркировке трафика (labeling errors), которая оценивается примерно в 6.67%. Это напрямую влияет на качество обучения классификаторов на сырых данных.
**Упоминание проблемы в обзорах (Liu et al.):** [arXiv:2402.10974](https://arxiv.org/html/2402.10974v1)

3. **Мультиколлинеарность и избыточность признаков (Feature Redundancy):**
Анализ извлеченных сетевых потоков CSE-CIC-IDS2018 показывает наличие огромных кластеров зависимых переменных. Многие метрики (например, счетчики пакетов и суммарные объемы байтов) имеют коэффициент корреляции >0.9. Как отмечается в фундаментальных обзорах датасета, прямое использование всех изначальных признаков без проведения корреляционного анализа (Feature Selection) приводит к переобучению (overfitting) алгоритмов и критическому перерасходу оперативной памяти.
**Исследование проблемы отбора признаков (Leevy et al., 2020):** [Journal of Big Data: Analysis of models based on CSE-CIC-IDS2018](https://journalofbigdata.springeropen.com/articles/10.1186/s40537-020-00382-x)
---
### Проблема 1. Утечка данных (Data Leakage) и коллинеарные дубликаты столбцов
...
### Проблема 2. Вшитые строки-заголовки и синтаксический мусор целевой переменной (`Label`)
...
### Проблема 3. Строковый мусор "Infinity" и "NaN" в скоростных характеристиках трафика
...
### Проблема 4. Преждевременное закрытие и фрагментация сессий по флагу FIN (Баг FIN)
...
### Проблема 5. Зависание сессий в памяти из-за игнорирования жесткого сброса RST (Баг RST) и физические дефекты знаковых полей
...
### Проблема 6. Конфликт таймаутов и хардкод лимитов сессий (120с против 600с)
...
### Проблема 7. Ошибочный учет Ethernet Padding (+6 байт из ниоткуда)
...
### Проблема 8. Двойной учет первого пакета (баг Average Packet Size = 9) и общие константные признаки

#### 1. Суть и природа проблемы

В Java-коде логики инициализации сетевой сессии CICFlowMeter допущена техническая ошибка: самый первый захваченный пакет добавляется в накопительный объект статистики дважды.

**Пример математического сбоя метрики `Pkt Size Avg` при передаче 2 пакетов по 6 байт:**

* **Прилетает 1-й пакет (6 байт):** из-за бага софт добавляет его два раза. Накопленная сумма длин = $6 + 6 = 12$. Счетчик пакетов в объекте статистики = 2.
* **Прилетает 2-й пакет (6 байт):** софт обрабатывает его корректно. Накопленная сумма длин = $12 + 6 = 18$. Количество пакетов для расчета среднего размера берется как фактическое количество пакетов в сессии (то есть 2).

Итог расчета метрики $\mathrm{Pkt\ Size\ Avg} = \frac{18}{2} = 9$. При этом соседний признак среднего значения длины пакета (`Pkt Len Mean`) рассчитывается через другую переменную, где данный баг отсутствует, и возвращает верное значение 6.

Взаимоисключающие математические признаки ломают логику построения ветвлений в древовидных моделях классификации (например, Random Forest и Gradient Boosting), так как создают ложные линейные зависимости. Дополнительно в файлах датасета присутствуют скрытые константные признаки (количество уникальных значений `nunique <= 1`), которые не имеют дисперсии, не несут полезной информации для разделения классов и замедляют сходимость алгоритмов.
...
# Формирование единого списка исключения
...
#### 3. Технический разбор и спецификация удаляемых признаков

В процессе выполнения алгоритма фильтрации из структуры данных исключаются 23 столбца. Ниже представлен подробный разбор каждой удаляемой колонки, сгруппированный по характеру дефекта.

##### 3.1. Признак с программным дефектом вычисления

* **Pkt Size Avg (или Packet Size Avg):** Средний размер пакета. Исключается из-за критического бага двойного учета первого пакета в сессии. Его сохранение приводит к дублированию информации признака `Pkt Len Mean`, но со смещением математического ожидания, что искажает оценку весов признаков.

##### 3.2. Общие константные признаки (инварианты со значением 0 во всех подвыборках)

Группа признаков, связанных со статистикой блочных передач (Bulk) и специфических флагов, которые в рамках топологии генерации трафика CSE-CIC-IDS2018 принимают строго константное значение 0:

* **Fwd Byts/b Avg (Forward Bytes/Bulk Avg):** Среднее количество байт в блоке передачи в прямом направлении. Равно 0 для всего датасета из-за отсутствия агрегации блоковых данных парсером.
* **Fwd Pkts/b Avg (Forward Packets/Bulk Avg):** Среднее количество пакетов в блоке передачи в прямом направлении. Постоянное нулевое значение.
* **Fwd Blk Rate Avg (Forward Bulk Rate Avg):** Средняя скорость передачи блоков в прямом направлении. Не имеет дисперсии.
* **Bwd Byts/b Avg (Backward Bytes/Bulk Avg):** Среднее количество байт в блоке передачи в обратном направлении. Константа 0.
* **Bwd Pkts/b Avg (Backward Packets/Bulk Avg):** Среднее количество пакетов в блоке передачи в обратном направлении. Константа 0.
* **Bwd Blk Rate Avg (Backward Bulk Rate Avg):** Средняя скорость передачи блоков в обратном направлении. Константа 0.
* **Bwd PSH Flags:** Количество пакетов с установленным флагом PUSH в обратном направлении. Из-за специфики съема трафика утилитой во всех сессиях принимает значение 0.
* **Fwd URG Flags:** Количество пакетов с установленным флагом URGENT в прямом направлении. Не содержит информативного распределения (строгая константа 0).
* **Bwd URG Flags:** Количество пакетов с установленным флагом URGENT в обратном направлении. Равно 0 во всех записях.
* **CWE Flag Count:** Количество пакетов с флагом Congestion Window Reduced. Принимает нулевое значение во всей генеральной совокупности.

##### 3.3. Служебные метаданные и сетевые идентификаторы (Список ALWAYS_REMOVE)

Данные признаки удаляются во избежание ложного обучения (overfitting) моделей на конкретную архитектуру тестового стенда и временные промежутки проведения эксперимента:

* **Flow ID:** Уникальный текстовый идентификатор сетевого потока. Содержит уникальные строковые значения для каждой сессии, не подлежит математическому обобщению.
* **Src IP (Source IP):** IP-адрес отправителя пакета. Исключается, чтобы модель не запоминала конкретные IP-адреса атакующих машин, зафиксированных на стенде, а обучалась на поведенческих характеристиках трафика.
* **Dst IP (Destination IP):** IP-адрес получателя пакета. Удаляется для предотвращения жесткой привязки классификатора к адресам серверов-жертв внутри инфраструктуры AWS.
* **Src Port (Source Port):** Исходный порт соединения. Чаще всего генерируется операционной системой динамически (динамические порты из диапазона 49152–65535), поэтому является случайным шумом для модели.
* **Timestamp:** Временная метка фиксации потока. Приводит к дрейфу данных (data drift) и ложной привязке сетевых атак к конкретному времени суток или дате проведения эксперимента.

Удаление указанных 23 колонок обеспечивает сокращение признакового пространства до 60 семантически независимых информативных переменных, гарантируя идентичность структуры данных во всех десяти обрабатываемых файлах.
